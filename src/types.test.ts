...
describe('Schema Types', () => {
  ...
  describe('Factory Functions', () => {
    it('number() creates NumberSchema with .as() method', () => {
      const _numSchema = number();
      type TestNumFactory = AssertExtends<typeof _numSchema, NumberSchema>;
      typeCheck<TestNumFactory>(true);
      // eslint-disable-next-line @typescript-eslint/no-unsafe-function-type
      type TestNumFactoryHasAs = typeof _numSchema extends { as: Function } ? true : false;
      typeCheck<TestNumFactoryHasAs>(true);
    });

    it('string() creates StringSchema with specific quote types', () => {
      const _strSchema = string(['"', "'"]);
      type TestStrFactory = AssertExtends<typeof _strSchema, StringSchema<readonly string[]>>;
      typeCheck<TestStrFactory>(true);
    });

    it('ident() creates IdentSchema', () => {
      const _identSchema = ident();
      type TestIdentFactory = AssertExtends<typeof _identSchema, IdentSchema>;
      typeCheck<TestIdentFactory>(true);
    });

    it('constVal() creates ConstSchema with the exact value type', () => {
      const _plusSchema = constVal('+');
      type TestConstFactory = AssertExtends<typeof _plusSchema, ConstSchema<'+'>>;
      typeCheck<TestConstFactory>(true);
    });

    it("lhs() creates ExprSchema with role 'lhs'", () => {
      const _lhsSchemaNum = lhs('number');
      type TestLhsFactory = AssertExtends<typeof _lhsSchemaNum, ExprSchema<'number', 'lhs'>>;
      typeCheck<TestLhsFactory>(true);
    });

    it("rhs() creates ExprSchema with role 'rhs'", () => {
      const _rhsSchemaNum = rhs('number');
      type TestRhsFactory = AssertExtends<typeof _rhsSchemaNum, ExprSchema<'number', 'rhs'>>;
      typeCheck<TestRhsFactory>(true);
    });

    it("expr() creates ExprSchema with role 'expr' (full grammar reset)", () => {
      const _exprSchemaNum = expr('number');
      type TestExprFactory = AssertExtends<typeof _exprSchemaNum, ExprSchema<'number', 'expr'>>;
  ...
  describe('NodeSchema', () => {
    ...
    it('defineNode creates a NodeSchema with exact types preserved', () => {
      ...
      type TestNodeSchema = AssertExtends<
        ...
        NodeSchema<
          'add',
          readonly [ExprSchema<'number', 'lhs'>, ConstSchema<'+'>, ExprSchema<'number', 'rhs'>],
          1,
          'number'
        >
      >;
...
describe('AST Node Types', () => {
  ...
  describe('ConstNode', () => {
    it('ConstNode<Value>: matched constant', () => {
      type TestConstNode = AssertEqual<ConstNode<'+'>, { node: 'const'; outputSchema: '+' }>;
  ...
  describe('BinaryNode', () => {
    it('BinaryNode has expected properties', () => {
      ...
      type TestBinaryNodeHasOutput = TestBinaryNodeShape['outputSchema'] extends 'number'
        ? true
        : false;
      type TestBinaryNodeHasLeft =
        TestBinaryNodeShape['left'] extends NumberNode<'1'> ? true : false;
      type TestBinaryNodeHasRight =
        TestBinaryNodeShape['right'] extends NumberNode<'2'> ? true : false;
...
describe('Grammar Types', () => {
  ...
  type TestNodes = readonly [typeof _add, typeof _mul];

  describe('Grammar (Flat Tuple Structure)', () => {
    it('Grammar is a tuple of node arrays', () => {
      type TestGrammarStructure = AssertExtends<Grammar, readonly (readonly NodeSchema[])[]>;
...
describe('Parse Type', () => {
  ...
  describe('Precedence', () => {
    it("'1+2*3' → Add(1, Mul(2, 3)) - mul binds tighter", () => {
      ...
      type Expected5 = [AddNode<NumberNode<'1'>, MulNode<NumberNode<'2'>, NumberNode<'3'>>>, ''];
    ...
    it("'1*3+2' → Add(Mul(1, 3), 2) - mul binds tighter", () => {
      ...
      type Expected6 = [AddNode<MulNode<NumberNode<'1'>, NumberNode<'3'>>, NumberNode<'2'>>, ''];
    ...
    it("'2*3*4' → Mul(2, Mul(3, 4)) - right-associative at same level", () => {
      ...
      type Expected7 = [MulNode<NumberNode<'2'>, MulNode<NumberNode<'3'>, NumberNode<'4'>>>, ''];
...
describe('Complex Examples', () => {
  ...
  it("Chained operations: '1+2+3' → Add(1, Add(2, 3))", () => {
    ...
    type Expected10 = [AddNode<NumberNode<'1'>, AddNode<NumberNode<'2'>, NumberNode<'3'>>>, ''];
  ...
  it("Mixed precedence chain: '1+2*3+4' → Add(1, Add(Mul(2, 3), 4))", () => {
    ...
    type Expected11 = [
      AddNode<NumberNode<'1'>, AddNode<MulNode<NumberNode<'2'>, NumberNode<'3'>>, NumberNode<'4'>>>,
      '',
    ];
