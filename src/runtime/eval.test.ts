...
const allNodes = [...arithmeticNodes, ...comparisonNodes, ...logicalNodes, ternary, concat] as const;
...
describe("evaluate - error cases", () => {
  ...
  it("should throw for node without eval function", () => {
    ...
    expect(() => {
      evaluate(
        { node: "noEval", outputSchema: "number", left: { node: "literal", value: 1, outputSchema: "number" }, right: { node: "literal", value: 2, outputSchema: "number" } },
